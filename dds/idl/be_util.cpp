...
be_util::usage()
{
  ...
  ACE_DEBUG((LM_DEBUG,
    ...
    ACE_TEXT(" --default-extensibility\t\tset default XTypes extensibility kind")
    ACE_TEXT(" (appendable if not set)\n")
    ACE_TEXT(" --default-data-representation\t\tset default XTypes data representation kind\n")
    ACE_TEXT(" \t\t\t\t\t(xcdr1, xcdr2, xml are all used if not set)\n)")
