...
void Message::block(ACE_Message_Block* block)
{
  ...
  block_ = ACE_Message_Block::duplicate(block);
}
...
Message& Message::operator=(const Message& rhs)
{
  if (&rhs != this) {
    ...
    ACE_Message_Block::release(block_);
    block_ = ACE_Message_Block::duplicate(rhs.block_);
...
bool Message::verify_message_integrity(const std::string& password) const
{
  ...
  for (STUN::Message::const_iterator pos = attributes_.begin(), limit = attributes_.end(); pos != limit; ++pos) {
    if (pos->type == STUN::MESSAGE_INTEGRITY) {
      ...
      compute_message_integrity_i(password, computed_message_integrity);
...
void Message::compute_message_integrity_i(const std::string& password, unsigned char message_integrity[20]) const
{
  ...
  // Compute the SHA1.
  TheSecurityRegistry->builtin_config()->get_utility()->hmac(message_integrity, amb->rd_ptr(), amb->length(), password);
