...
bool RelayStatusReporter::setup_writer()
{
  ...
  return setup_writer_i();
}

bool RelayStatusReporter::setup_writer_i()
{
  ...
  if (publish_relay_status_liveliness_ != OpenDDS::DCPS::TimeDuration::zero_value) {
    relay_status_writer_qos.liveliness.lease_duration = config_.publish_relay_status_liveliness().to_dds_duration();
  ...
  reactor()->schedule_timer(this, 0, ACE_Time_Value(), config_.publish_relay_status().value());
  return true;
}
  ...
  for (size_t idx = 0; idx != samples.size(); ++idx) {
    ...
    if (info.valid_data) {
      if (pair.key() == RTPS_RELAY_PUBLISH_RELAY_STATUS) {
        ...
        OpenDDS::DCPS::TimeDuration td;
        if (Config::to_time_duration(pair.value(), td)) {
          if (td != publish_relay_status_liveliness_) {
